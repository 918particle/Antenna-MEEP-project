...
def calculate_radiation_pattern(simulation, n2f_objs):
    ...
    for i in range(N):
        ...
        for n in range(constants.npts):
            ff = simulation.get_farfield(
                n2f_objs[i],
                mp.Vector3(
                    constants.r * np.cos(angles[n]), constants.r * np.sin(angles[n])
                ),
            )
        ...
        directivity = 10.0 * np.log10(Pr) - 10.0 * np.log10(constants.normalization)  # same as 10log10(Pr/normalization)
...
def locate_beams(rad_patterns):
    ...
    for pattern in rad_patterns:
        ...
        for i in range(1, constants.npts):
            if (
                not In
                and (m - p[i]) <= constants.beam_threshold
                and (m - p[i - 1]) >= constants.beam_threshold
            ):
                In = True
                current_lobe.append(angles[i]) if angles[
                    i
                ] <= np.pi else current_lobe.append(angles[i] - 2 * np.pi)
                ...
                In = False
                results.append(
                    (np.mean(current_lobe), np.max(current_power))
                ) if current_lobe else None
                current_lobe = []
                current_power = []
    return results
