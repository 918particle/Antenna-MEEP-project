...
for f in frequencies:
    ...
    region = Near2Far_Region(
        c.n2f_x_size, c.n2f_y_size, np.array([c.base_frequency, f])
    )
    ...
    for i in range(len(results[1][1])):
        all_results.append(
            {
                "frequency": float(f),
                "directivity": float(results[1][1][i]),
                "angles": float(results[1][0][i]),
            }
        )
    ...
    # graph_patterns.plot_radiation_patterns_polar(results, f)
    # Plot surfaces
    # if f == frequencies[0]:
...
# graph_patterns.plot_radiation_patterns_polar_plotly(all_results)
