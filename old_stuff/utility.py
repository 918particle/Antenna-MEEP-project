...
def make_near_to_far_field_box(sx, sy, xoffset, yoffset, frequency, sim):
    top = mp.Near2FarRegion(
        center=mp.Vector3(xoffset, 0.5 * sy + yoffset, 0),
        size=mp.Vector3(sx, 0, sy),
        weight=+1,
    )
    ...
    right = mp.Near2FarRegion(
        center=mp.Vector3(0.5 * sx + xoffset, yoffset, 0),
        size=mp.Vector3(0, sy, sy),
        weight=+1,
    )
...
def make_flux_region(x, y, lx, ly, sim):
    ...
    flux_monitor_volume = mp.FluxRegion(
        center=mp.Vector3(x, y, 0), size=mp.Vector3(lx, 0, 0), direction=-mp.Y
    )
...
def calculate_radiation_pattern(sim, projection_box, E_or_H_Plane):
    ...
    if E_or_H_Plane:
        ...
        for n in range(npts):
            ff = sim.get_farfield(
                projection_box,
                mp.Vector3(r * np.cos(angles[n]), r * np.sin(angles[n]), 0),
            )
        ...
        for n in range(npts):
            ff = sim.get_farfield(
                projection_box,
                mp.Vector3(r * np.sin(angles[n]), 0, r * np.cos(angles[n])),
            )
            E[n, :] = [ff[j] for j in range(3)]
            H[n, :] = [ff[j + 3] for j in range(3)]
        Px = np.real(np.conj(E[:, 1]) * H[:, 2] - np.conj(E[:, 2]) * H[:, 1])
